...
class TestContainerCrossInitramfs:
    """Test container-cross-initramfs build."""

    @pytest.mark.slow
    def test_initramfs_build(self, build_dir, machine):
        """Test that container-cross-initramfs-build succeeds."""
        result = run_bitbake(
            build_dir,
            "container-cross-initramfs-build",
            timeout=3600,
        )
        assert result.returncode == 0, f"Build failed: {result.stderr}"

    def test_blobs_exist(self, deploy_dir):
        """Test that initramfs blobs were deployed."""
        blob_dir = deploy_dir / "container-cross-initramfs"

        # Check for kernel
        kernel_files = list(blob_dir.glob("*Image")) + list(blob_dir.glob("bzImage"))
        assert len(kernel_files) > 0, f"No kernel found in {blob_dir}"

        # Check for initramfs
        initramfs_files = list(blob_dir.glob("*.cpio.gz"))
        assert len(initramfs_files) > 0, f"No initramfs found in {blob_dir}"
...
class TestVdkrRecipes:
    """Test vdkr recipe builds."""

    def test_vdkr_native_build(self, build_dir):
        """Test that vdkr-native builds."""
        result = run_bitbake(build_dir, "vdkr-native")
        assert result.returncode == 0, f"Build failed: {result.stderr}"

    @pytest.mark.slow
    def test_vcontainer_tarball(self, build_dir):
        """Test creating vcontainer standalone tarball."""
        result = run_bitbake(
            build_dir,
            "vcontainer-native",
            task="create_tarball",
        ...
        vdkr_deploy = build_dir / "tmp" / "deploy" / "vdkr"
        tarballs = list(vdkr_deploy.glob("vcontainer-standalone-*.tar.gz"))
        assert len(tarballs) > 0, f"No tarball found in {vdkr_deploy}"
