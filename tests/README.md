...
# 3. Build the standalone tarball (includes blobs + QEMU)
MACHINE=qemux86-64 bitbake vdkr-native -c create_tarball
# Output: tmp/deploy/vdkr/vdkr-standalone-x86_64.tar.gz

# 4. Extract the tarball
cd /tmp
rm -rf vdkr-standalone
tar -xzf /opt/bruce/poky/build/tmp/deploy/vdkr/vdkr-standalone-*.tar.gz
...
./tests/memres-test.sh stop --vdkr-dir /tmp/vdkr-standalone

# Run only fast tests (skip network/slow tests)
pytest tests/test_vdkr.py -v -m "not slow and not network" --vdkr-dir /tmp/vdkr-standalone

# Run specific test class
pytest tests/test_vdkr.py::TestMemresBasic -v --vdkr-dir /tmp/vdkr-standalone
...
./tests/memres-test.sh stop --vdkr-dir /tmp/vcontainer-standalone --tool vpdmn

# Run only fast tests (skip network/slow tests)
pytest tests/test_vpdmn.py -v -m "not slow and not network" --vdkr-dir /tmp/vcontainer-standalone

# Run specific test class
pytest tests/test_vpdmn.py::TestVrun -v --vdkr-dir /tmp/vcontainer-standalone
```
...
pytest tests/test_vdkr.py tests/test_vpdmn.py -v -m "not slow and not network" --vdkr-dir /tmp/vcontainer-standalone
```
...
# 4. (Optional) Build initramfs blobs if testing container bundling:
...
|------------|---------------|----------------|
| `TestContainerCrossInitramfs` | Initramfs blob building | `bitbake container-cross-initramfs-build` |
...
pytest tests/test_vpdmn.py --vdkr-dir /tmp/vcontainer-standalone 2>&1 | tee /tmp/pytest-output.log
...
```bash
# 1. Build the unified standalone tarball (includes both vdkr and vpdmn)
cd /opt/bruce/poky
source oe-init-build-env
...
cd /tmp
rm -rf vcontainer-standalone
tar -xzf /opt/bruce/poky/build/tmp/deploy/vdkr/vcontainer-standalone-*.tar.gz

# 3. Run fast tests for both tools (skips network and slow tests)
cd /opt/bruce/poky/meta-virtualization
pytest tests/test_vdkr.py tests/test_vpdmn.py -v --vdkr-dir /tmp/vcontainer-standalone -m "not network and not slow"
...
```bash
# Build vdkr standalone
MACHINE=qemux86-64 bitbake vdkr-native -c create_tarball

# Extract
cd /tmp && rm -rf vdkr-standalone
tar -xzf /opt/bruce/poky/build/tmp/deploy/vdkr/vdkr-standalone-*.tar.gz
