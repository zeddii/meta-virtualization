...
Usage:
    # Run all tests
    pytest tests/ --vdkr-dir /tmp/vcontainer-standalone
    ...
    ./tests/memres-test.sh start --vdkr-dir /tmp/vcontainer-standalone
    pytest tests/test_vdkr.py --vdkr-dir /tmp/vcontainer-standalone --skip-destructive

    # Run specific test
    pytest tests/test_vdkr.py::TestMemresBasic -v --vdkr-dir /tmp/vcontainer-standalone
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--vdkr-dir",
        action="store",
        default=os.environ.get("VDKR_STANDALONE_DIR", "/tmp/vdkr-standalone"),
        help="Path to vdkr standalone directory",
    )
    parser.addoption(
        "--arch",
        action="store",
