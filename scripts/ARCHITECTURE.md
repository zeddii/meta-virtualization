...
- **`go-mod-vcs.bbclass`**
  ...
  Also provides:
  - `go_mod_fix_permissions` prefunc: Runs before `do_unpack` to fix Go module cache read-only permissions
  - `do_fix_go_mod_permissions` task: Runs after `do_sync_go_files` to ensure cleanup can succeed
  - `do_go_mod_recommend` task: Analyzes VCS module sizes and suggests hybrid conversion

- **`oe-go-mod-fetcher-hybrid.py`**
  Hybrid mode converter. Transforms VCS-only recipes into hybrid `gomod://` + `git://` recipes for faster builds:
  - `--git "prefixes"`: Keep specified module prefixes as `git://` (VCS provenance)
  - `--gomod "prefixes"`: Convert specified prefixes to `gomod://` (fast proxy fetch)
  - `--recommend`: Analyze module sizes and suggest conversion
  - Generates `go-mod-hybrid-gomod.inc`, `go-mod-hybrid-git.inc`, `go-mod-hybrid-cache.inc`
  - Fetches SHA256 checksums from proxy.golang.org in parallel
  - Uses inline checksums to avoid BitBake parse errors with special characters (e.g., `~`)

## Hybrid Mode Architecture

The hybrid mode allows mixing `gomod://` (fast proxy downloads) with `git://` (VCS provenance) in a single recipe. This is particularly useful for large projects like k3s where full VCS fetch of 1000+ modules is slow.
...
│ Hybrid .inc files                                                        │
│ ┌─────────────────────────┐ ┌──────────────────────┐ ┌────────────────┐ │
│ │go-mod-hybrid-gomod.inc  │ │go-mod-hybrid-git.inc │ │go-mod-hybrid-  │ │
│ │(gomod:// + sha256sum)   │ │(git:// selected)     │ │cache.inc       │ │
│ └─────────────────────────┘ └──────────────────────┘ └────────────────┘ │
└────────────────────────────────┬────────────────────────────────────────┘
                                 │
         ┌───────────────────────┼───────────────────────┐
...
Recipes can support both modes via conditional includes:
...
### Module Cache Permission Handling

Go's module cache intentionally creates read-only files for integrity. This conflicts with BitBake's `do_unpack` cleanup which uses `rm -rf`. The class provides two-pronged permission fixes:

1. **Before unpack** (`go_mod_fix_permissions` prefunc): Runs before `do_unpack`'s `cleandirs` removes `${S}`, making cached files writable so cleanup succeeds.

2. **After module creation** (`do_fix_go_mod_permissions` task): Runs after `do_sync_go_files` to ensure the newly created cache is writable for subsequent cleanup.

Both functions check multiple paths:
- `${S}/pkg/mod`
- `${S}/src/import/pkg/mod` (k3s-style layout)
- `${WORKDIR}/sources/*/pkg/mod`

### Checksum Handling

The hybrid script fetches SHA256 checksums from `proxy.golang.org` using parallel downloads (8 workers). Checksums are embedded inline in the SRC_URI:

```bitbake
# Inline checksum avoids BitBake variable flag parse issues
SRC_URI += "gomod://git.sr.ht/~sbinet/gg;version=v0.5.0;sha256sum=abc123..."
```

This approach avoids the alternative `SRC_URI[name.sha256sum]` syntax which fails for module paths containing special characters like `~`.
