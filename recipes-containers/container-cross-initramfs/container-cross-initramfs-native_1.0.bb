...
# When using CONTAINER_CROSS_USE_DEPLOY=1, we need to invalidate sstate when
# DEPLOY_DIR blobs change. We do this by:
# 1. Making do_install depend on blob file checksums
# 2. Disabling sstate for this recipe when in deploy mode
#
# This ensures the recipe rebuilds when blobs are updated.

# Disable sstate when using DEPLOY_DIR to ensure we always pick up latest blobs
SSTATE_SKIP_CREATION:task-install = "${@'1' if d.getVar('CONTAINER_CROSS_USE_DEPLOY') == '1' else '0'}"

# Force reinstall when in deploy mode by making task always run
do_install[nostamp] = "${@'1' if d.getVar('CONTAINER_CROSS_USE_DEPLOY') == '1' else '0'}"
