...
if [ "$BATCH_IMPORT" = "true" ]; then
    ...
    # Each container: skopeo copy oci:/input/N docker-daemon:image:tag
    COMPOUND_CMD=""
    for i in "${!BATCH_IMAGES[@]}"; do
        img="${BATCH_IMAGES[$i]}"
        if [ "$RUNTIME" = "docker" ]; then
            CMD="skopeo copy oci:/input/$i docker-daemon:$img"
        else
            CMD="skopeo copy oci:/input/$i containers-storage:$img"
        fi
