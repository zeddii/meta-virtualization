...
#   2. VDKR_CONFIG_DIR environment variable
#   3. Default: ~/.config/vdkr
#
...
CONFIG_DIR="${_PREPARSE_CONFIG_DIR:-${VDKR_CONFIG_DIR:-$HOME/.config/vdkr}}"
...
# 2. Executable name: vdkr-aarch64 -> aarch64, vdkr-x86_64 -> x86_64
...
detect_arch_from_name() {
    ...
    case "$prog_name" in
        vdkr-aarch64) echo "aarch64" ;;
        vdkr-x86_64)  echo "x86_64" ;;
        *)            echo "" ;;
    esac
}
...
# Runner script
RUNNER="${VDKR_RUNNER:-$SCRIPT_DIR/vrunner.sh}"
...
check_oci_arch() {
    ...
    if [ -z "$image_arch" ]; then
        ...
        echo -e "${YELLOW}[vdkr]${NC} Warning: Could not determine image architecture" >&2
        return 0
    fi
    ...
    if [ "$normalized_image_arch" != "$normalized_target_arch" ]; then
        echo -e "${RED}[vdkr]${NC} Architecture mismatch!" >&2
        echo -e "${RED}[vdkr]${NC}   Image architecture: ${BOLD}$image_arch${NC}" >&2
        echo -e "${RED}[vdkr]${NC}   Target architecture: ${BOLD}$target_arch${NC}" >&2
        echo -e "${YELLOW}[vdkr]${NC} Use --arch $image_arch to import to a matching environment" >&2
        return 1
    fi

    [ "$VERBOSE" = "true" ] && echo -e "${GREEN}[vdkr]${NC} Image architecture: $image_arch (matches target)" >&2
    return 0
}
...
${BOLD}RUN vs VRUN:${NC}
    ${CYAN}run${NC}   - Full Docker passthrough. Entrypoint is honored.
    ...
    Use 'run' when you need --entrypoint, -e, --rm, or other docker options.
...
${BOLD}CONFIGURATION (vconfig):${NC}
    ...
    Supported keys: arch, timeout, state-dir, verbose
    Config file: \$CONFIG_DIR/config (default: ~/.config/vdkr/config)

${BOLD}GLOBAL OPTIONS:${NC}
    --arch, -a <arch>     Target architecture: x86_64 or aarch64 [default: ${DEFAULT_ARCH}]
    --config-dir <path>   Configuration directory [default: ~/.config/vdkr]
    ...
    --state-dir <path>    Override state directory [default: ~/.vdkr/<arch>]
...
${BOLD}DOCKER RUN/VRUN OPTIONS:${NC}
    All docker run options are passed through (e.g., -it, -e, -p, --rm, etc.)
...
${BOLD}EXAMPLES:${NC}
    ...
    # Run a command in a container (docker-compatible syntax)
    ${PROG_NAME} run alpine /bin/echo hello
    ${PROG_NAME} run --rm alpine uname -m      # Check container architecture

    # Interactive shell (docker-compatible syntax)
    ${PROG_NAME} run -it alpine /bin/sh

    # With environment variables and other docker options
...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        --state-dir)
            ...
            ;;
        --instance|-I)
            # Shortcut: -I web expands to --state-dir ~/.vdkr/web
...
run_docker_command() {
    ...
    if daemon_is_running; then
        ...
        [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Using daemon mode" >&2
        "$RUNNER" $runner_args --daemon-send "$docker_cmd"
    else
...
run_docker_command_with_input() {
    ...
    if daemon_is_running; then
        ...
        [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Using daemon mode for file I/O" >&2
        "$RUNNER" $runner_args --input "$input_path" --input-type "$input_type" --daemon-send-input -- "$docker_cmd"
    else
        # Regular mode - start QEMU for this command
        "$RUNNER" $runner_args --input "$input_path" --input-type "$input_type" -- "$docker_cmd"
    fi
}
...
prepare_volume() {
    ...
    if [ ! -e "$host_path" ]; then
        echo -e "${RED}[vdkr]${NC} Volume source not found: $host_path" >&2
        return 1
    fi
    ...
    if [ -d "$host_path" ]; then
        ...
        [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Volume: copied directory $host_path -> /mnt/share/volumes/$hash" >&2
    else
        # File: copy file
        cp -L "$host_path" "$vol_dir/"
        [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Volume: copied file $host_path -> /mnt/share/volumes/$hash" >&2
    fi
    ...
    for i in "${!VOLUME_MOUNTS[@]}"; do
        ...
        if [ "$mode" = "rw" ]; then
            ...
            if [ -d "$vol_dir" ] && [ -d "$host_path" ]; then
                [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Syncing volume back: $vol_dir -> $host_path" >&2
    ...
    while [ $i -lt ${#args[@]} ]; do
        ...
        case "$arg" in
            -v|--volume)
                ...
                i=$((i + 1))
                if [ $i -ge ${#args[@]} ]; then
                    echo -e "${RED}[vdkr]${NC} -v requires an argument" >&2
                    return 1
                fi
...
case "$COMMAND" in
    images)
        # docker images
        run_docker_command "docker images ${COMMAND_ARGS[*]}"
        ;;
    ...
    pull)
        ...
        if [ ${#COMMAND_ARGS[@]} -lt 1 ]; then
            echo -e "${RED}[vdkr]${NC} pull requires <image>" >&2
            exit 1
        fi
        ...
        if daemon_is_running; then
            ...
            run_docker_command "docker pull $IMAGE_NAME && docker images"
        else
            # Regular mode - need to enable networking
            NETWORK="true"
            RUNNER_ARGS=$(build_runner_args)
            "$RUNNER" $RUNNER_ARGS -- "docker pull $IMAGE_NAME && docker images"
        fi
        ;;
    ...
    load)
        ...
        run_docker_command_with_input "$INPUT_FILE" "tar" \
    ...
    import)
        ...
        run_docker_command_with_input "$INPUT_PATH" "tar" \
    ...
    vimport)
        ...
        if [ -d "$INPUT_PATH" ]; then
            if [ -f "$INPUT_PATH/index.json" ] || [ -f "$INPUT_PATH/oci-layout" ]; then
                ...
                DOCKER_CMD="skopeo copy oci:{INPUT} docker-daemon:$IMAGE_NAME && docker images"
            else
        ...
        run_docker_command_with_input "$INPUT_PATH" "$INPUT_TYPE" "$DOCKER_CMD"
        ;;
    ...
    save)
        ...
        if daemon_is_running; then
            ...
            [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Using daemon mode for save" >&2
            ...
            run_docker_command "docker save -o /mnt/share/output.tar $IMAGE_NAME"
            ...
            if [ -f "$SHARE_DIR/output.tar" ]; then
                ...
                echo -e "${GREEN}[vdkr]${NC} Saved to $OUTPUT_FILE ($(du -h "$OUTPUT_FILE" | cut -f1))"
            else
                echo -e "${RED}[vdkr]${NC} Save failed - output not found in shared directory" >&2
                exit 1
            fi
        else
    ...
    tag|rmi)
        ...
        run_docker_command "docker $COMMAND ${COMMAND_ARGS[*]}"
        ;;

    # Container lifecycle commands
    ps)
        # List containers
        run_docker_command "docker ps ${COMMAND_ARGS[*]}"
        ;;

    rm)
        # Remove containers
        run_docker_command "docker rm ${COMMAND_ARGS[*]}"
        ;;

    logs)
        # View container logs
        run_docker_command "docker logs ${COMMAND_ARGS[*]}"
        ;;

    inspect)
        # Inspect container or image
        run_docker_command "docker inspect ${COMMAND_ARGS[*]}"
        ;;
    ...
    commit)
        ...
        run_docker_command "docker commit ${COMMAND_ARGS[*]}"
        ;;

    history)
        # Show image history
        run_docker_command "docker history ${COMMAND_ARGS[*]}"
        ;;

    # Registry commands
    push)
        # Push image to registry
        run_docker_command "docker push ${COMMAND_ARGS[*]}"
        ;;

    search)
        # Search registries
        run_docker_command "docker search ${COMMAND_ARGS[*]}"
        ;;

    login)
        # Login to registry - may need credentials via stdin
        # For non-interactive: docker login -u user -p pass registry
        run_docker_command "docker login ${COMMAND_ARGS[*]}"
        ;;

    logout)
        # Logout from registry
        run_docker_command "docker logout ${COMMAND_ARGS[*]}"
        ;;

    # Docker exec - execute command in running container
    exec)
        if [ ${#COMMAND_ARGS[@]} -lt 2 ]; then
            echo -e "${RED}[vdkr]${NC} exec requires <container> <command>" >&2
            exit 1
        fi
        ...
        if [ "$EXEC_INTERACTIVE" = "true" ]; then
            ...
            if daemon_is_running; then
                ...
                [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Using daemon interactive mode" >&2
                RUNNER_ARGS=$(build_runner_args)
                "$RUNNER" $RUNNER_ARGS --daemon-interactive -- "docker exec ${EXEC_ARGS[*]}"
            else
                # No daemon running, use regular QEMU
                RUNNER_ARGS=$(build_runner_args)
                "$RUNNER" $RUNNER_ARGS -- "docker exec ${EXEC_ARGS[*]}"
            fi
        else
            # Non-interactive exec via daemon
            run_docker_command "docker exec ${EXEC_ARGS[*]}"
        fi
        ;;
    ...
    cp)
        if [ ${#COMMAND_ARGS[@]} -lt 2 ]; then
            echo -e "${RED}[vdkr]${NC} cp requires <src> <dest>" >&2
            echo "Usage: vdkr cp <container>:<path> <local_path>" >&2
            echo "       vdkr cp <local_path> <container>:<path>" >&2
            exit 1
        fi
        ...
        if [[ "$SRC" == *":"* ]] && [[ "$DEST" != *":"* ]]; then
            # Container to host: docker cp container:/path /local/path
            # Run docker cp to /mnt/share, then copy from share to host
            CONTAINER_PATH="$SRC"
            HOST_PATH="$DEST"
            SHARE_DIR="${STATE_DIR:-$DEFAULT_STATE_DIR/$TARGET_ARCH}/share"
            ...
            if daemon_is_running; then
                rm -rf "$SHARE_DIR"/* 2>/dev/null || true
                run_docker_command "docker cp $CONTAINER_PATH /mnt/share/"
                ...
                if [ -n "$(ls -A "$SHARE_DIR" 2>/dev/null)" ]; then
                    ...
                    rm -rf "$SHARE_DIR"/*
                    echo -e "${GREEN}[vdkr]${NC} Copied to $HOST_PATH"
                else
                    echo -e "${RED}[vdkr]${NC} Copy failed - no files in share directory" >&2
                    exit 1
                fi
            else
                echo -e "${RED}[vdkr]${NC} cp requires daemon mode. Start with: vdkr start" >&2
                exit 1
            fi

        elif [[ "$SRC" != *":"* ]] && [[ "$DEST" == *":"* ]]; then
            # Host to container: docker cp /local/path container:/path
            HOST_PATH="$SRC"
            CONTAINER_PATH="$DEST"

            if [ ! -e "$HOST_PATH" ]; then
                echo -e "${RED}[vdkr]${NC} Source not found: $HOST_PATH" >&2
                exit 1
            fi
            ...
            if daemon_is_running; then
                ...
                run_docker_command "docker cp /mnt/share/$BASENAME $CONTAINER_PATH"
                rm -rf "$SHARE_DIR"/*
                echo -e "${GREEN}[vdkr]${NC} Copied to $CONTAINER_PATH"
            else
                echo -e "${RED}[vdkr]${NC} cp requires daemon mode. Start with: vdkr start" >&2
                exit 1
            fi
        else
            echo -e "${RED}[vdkr]${NC} Invalid cp syntax. One path must be container:path" >&2
            exit 1
        fi
        ;;
    ...
    vconfig)
        ...
        if [ ${#COMMAND_ARGS[@]} -lt 1 ]; then
            # Show all config
            echo "vdkr configuration ($CONFIG_FILE):"
            echo ""
            ...
            if ! echo "$VALID_KEYS" | grep -qw "$KEY"; then
                echo -e "${RED}[vdkr]${NC} Unknown config key: $KEY" >&2
            ...
            if [ -z "$VALUE" ]; then
                ...
                if [ "$KEY" = "arch" ]; then
                    case "$VALUE" in
                        aarch64|x86_64) ;;
                        *)
                            echo -e "${RED}[vdkr]${NC} Invalid architecture: $VALUE" >&2
                ...
                if [ "$KEY" = "verbose" ]; then
                    case "$VALUE" in
                        true|false) ;;
                        *)
                            echo -e "${RED}[vdkr]${NC} Invalid verbose value: $VALUE" >&2
    ...
    clean)
        ...
        echo -e "${YELLOW}[vdkr]${NC} DEPRECATED: 'clean' is deprecated, use 'vstorage clean' instead" >&2
        echo -e "${YELLOW}[vdkr]${NC}   vstorage clean          - clean current architecture" >&2
        echo -e "${YELLOW}[vdkr]${NC}   vstorage clean <arch>   - clean specific architecture" >&2
        echo -e "${YELLOW}[vdkr]${NC}   vstorage clean --all    - clean all architectures" >&2
        echo "" >&2
        ...
        if [ -d "$CLEAN_DIR" ]; then
            ...
            if [ -f "$CLEAN_DIR/daemon.pid" ]; then
                ...
                if [ -n "$pid" ] && [ -d "/proc/$pid" ]; then
                    echo -e "${YELLOW}[vdkr]${NC} Stopping memres (PID $pid)..."
    ...
    system)
        # Passthrough to docker system commands (df, prune, events, etc.)
        if [ ${#COMMAND_ARGS[@]} -lt 1 ]; then
            echo -e "${RED}[vdkr]${NC} system requires a subcommand: df, prune, events, info" >&2
            exit 1
        fi
        run_docker_command "docker system ${COMMAND_ARGS[*]}"
        ;;
        ...
        case "$STORAGE_CMD" in
            ...
            df)
                ...
                for state_dir in "$DEFAULT_STATE_DIR"/*/; do
                    ...
                    for item in docker-state.img share; do
            ...
            clean)
                ...
                if [ "$arch" = "--all" ]; then
                    ...
                    echo -e "${YELLOW}[vdkr]${NC} Removing all storage directories..."
                    rm -rf "$DEFAULT_STATE_DIR"
                    echo -e "${GREEN}[vdkr]${NC} All storage cleaned."
                elif [ -n "$arch" ]; then
                    # Clean specific arch
                    ...
                    if [ -d "$clean_dir" ]; then
                        ...
                        rm -rf "$clean_dir"
                        echo -e "${GREEN}[vdkr]${NC} Cleaned: $clean_dir"
                    else
                        echo -e "${YELLOW}[vdkr]${NC} Not found: $clean_dir"
                    fi
                else
                    ...
                    if [ -d "$clean_dir" ]; then
                        ...
                        echo -e "${GREEN}[vdkr]${NC} No storage directory found for $TARGET_ARCH"
                    fi
                fi
                ;;

            *)
                echo -e "${RED}[vdkr]${NC} Unknown vstorage subcommand: $STORAGE_CMD" >&2
                echo "Usage: vdkr vstorage [list|path|df|clean]" >&2
                echo "" >&2
                echo "Subcommands:" >&2
    ...
    vrun)
        # Extended run: run a command in a container (docker-like syntax)
        # Usage: vdkr vrun [options] <image> [command] [args...]
        # Options:
        ...
        #
        # Parse vrun-specific options (allows docker-like: vdkr vrun -it alpine /bin/sh)
        VRUN_VOLUMES=()
        HAS_VOLUMES=false
        ...
        while [ ${#COMMAND_ARGS[@]} -gt 0 ]; do
            case "${COMMAND_ARGS[0]}" in
                ...
                -p|--publish)
                    ...
                    if [ ${#COMMAND_ARGS[@]} -lt 2 ]; then
                        echo -e "${RED}[vdkr]${NC} -p requires <host_port>:<container_port>" >&2
                        exit 1
                    fi
                ...
                -v|--volume)
                    ...
                    if [ ${#COMMAND_ARGS[@]} -lt 2 ]; then
                        echo -e "${RED}[vdkr]${NC} -v requires <host_path>:<container_path>" >&2
                        exit 1
                    fi
        ...
        if [ "$HAS_VOLUMES" = "true" ] && ! daemon_is_running; then
            echo -e "${RED}[vdkr]${NC} Volume mounts require daemon mode. Start with: vdkr memres start" >&2
            exit 1
        fi

        if [ ${#COMMAND_ARGS[@]} -lt 1 ]; then
            echo -e "${RED}[vdkr]${NC} vrun requires <image> [command] [args...]" >&2
            echo "Usage: vdkr vrun [options] <image> [command] [args...]" >&2
            echo "" >&2
            echo "Options:" >&2
            ...
            echo "" >&2
            echo "Examples:" >&2
            echo "  vdkr vrun alpine /bin/ls -la" >&2
            echo "  vdkr vrun -it alpine /bin/sh" >&2
            echo "  vdkr vrun -p 8080:80 nginx:latest" >&2
            echo "  vdkr vrun -v /tmp/data:/data alpine cat /data/file.txt" >&2
            exit 1
        fi
        ...
        if [ "$HAS_VOLUMES" = "true" ]; then
            [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Preparing volume mounts..." >&2
            ...
            [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Volume options: $VOLUME_OPTS" >&2
        fi
        ...
        if [ "$NETWORK" = "true" ]; then
            DOCKER_RUN_OPTS="$DOCKER_RUN_OPTS --network=host --dns=10.0.2.3 --dns=8.8.8.8"
        fi

        # Add volume mounts
        if [ -n "$VOLUME_OPTS" ]; then
            DOCKER_RUN_OPTS="$DOCKER_RUN_OPTS $VOLUME_OPTS"
        fi
        ...
        if [ -n "$CONTAINER_CMD" ]; then
            ...
            DOCKER_CMD="docker run $DOCKER_RUN_OPTS --entrypoint '' $IMAGE_NAME $CONTAINER_CMD"
        else
            DOCKER_CMD="docker run $DOCKER_RUN_OPTS $IMAGE_NAME"
        fi

        [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Docker command: $DOCKER_CMD" >&2
        ...
        if [ "$INTERACTIVE" = "true" ]; then
            ...
            if [ "$HAS_VOLUMES" = "false" ] && daemon_is_running; then
                ...
                [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Using daemon interactive mode" >&2
                RUNNER_ARGS=$(build_runner_args)
                "$RUNNER" $RUNNER_ARGS --daemon-interactive -- "$DOCKER_CMD"
                exit $?
            else
                ...
                if daemon_is_running; then
                    ...
                    echo -e "${YELLOW}[vdkr]${NC} Stopping daemon for interactive mode..." >&2
                ...
                if [ "$DAEMON_WAS_RUNNING" = "true" ]; then
                    echo -e "${CYAN}[vdkr]${NC} Restarting daemon..." >&2
    ...
    run)
        # Docker run command - mirrors 'docker run' syntax
        # Usage: vdkr run [options] <image> [command]
        # Automatically prepends 'docker run' to the arguments
        # Supports volume mounts with -v (requires daemon mode)
        if [ ${#COMMAND_ARGS[@]} -eq 0 ]; then
            echo -e "${RED}[vdkr]${NC} run requires an image" >&2
            echo "Usage: vdkr run [options] <image> [command]" >&2
            echo "" >&2
            echo "Examples:" >&2
            echo "  vdkr run alpine /bin/echo hello" >&2
            echo "  vdkr run -it alpine /bin/sh" >&2
            echo "  vdkr run --rm -e FOO=bar myapp:latest" >&2
            echo "  vdkr run -v /tmp/data:/data alpine cat /data/file.txt" >&2
            exit 1
        fi
        ...
        if [ "$RUN_HAS_VOLUMES" = "true" ]; then
            [ "$VERBOSE" = "true" ] && echo -e "${CYAN}[vdkr]${NC} Preparing volume mounts for run command..." >&2
        ...
        # Build docker run command from args
        # Note: -it may have been consumed by global parser, so add it back if INTERACTIVE is set
        if [ "$INTERACTIVE" = "true" ]; then
            DOCKER_CMD="docker run -it ${COMMAND_ARGS[*]}"
        else
            DOCKER_CMD="docker run ${COMMAND_ARGS[*]}"
        fi
    ...
    # Memory resident subcommand: vdkr memres start|stop|restart|status
    # vmemres is the preferred name (v prefix for vdkr-specific commands)
    memres|vmemres)
        if [ ${#COMMAND_ARGS[@]} -lt 1 ]; then
            echo -e "${RED}[vdkr]${NC} memres requires a subcommand: start, stop, restart, status, list" >&2
            exit 1
        fi
        ...
        case "$MEMRES_CMD" in
            start)
                if daemon_is_running; then
                    echo -e "${YELLOW}[vdkr]${NC} A memres instance is already running for $TARGET_ARCH"
                    echo ""
                    ...
                    case "$choice" in
                        1)
                            echo -e "${CYAN}[vdkr]${NC} Restarting memres..."
                        ...
                        2)
                            ...
                            echo "  vdkr -I web memres start -p 8080:80"
                            echo "  vdkr -I api memres start -p 3000:3000"
                            echo ""
                            echo "Then interact with it:"
                            echo "  vdkr -I web images"
                            echo ""
                            exit 0
                            ;;
                        *)
            ...
            restart)
                ...
                for arg in "${COMMAND_ARGS[@]:1}"; do
                    if [ "$arg" = "--clean" ]; then
                        CLEAN_DIR="${STATE_DIR:-$DEFAULT_STATE_DIR/$TARGET_ARCH}"
                        if [ -d "$CLEAN_DIR" ]; then
                            echo -e "${YELLOW}[vdkr]${NC} Cleaning state directory: $CLEAN_DIR"
                            rm -rf "$CLEAN_DIR"
                        fi
                        break
                    fi
            ...
            list)
                ...
                for qemu_pid in $(pgrep -f "qemu-system.*runtime=(docker|podman)" 2>/dev/null || true); do
                    ...
                    # Also check vpdmn state dirs
                    vpdmn_tracked=false
                    for vpid_file in "$HOME/.vpdmn"/*/daemon.pid; do
                ...
                ;;
            *)
                echo -e "${RED}[vdkr]${NC} Unknown memres subcommand: $MEMRES_CMD" >&2
                echo "Usage: vdkr memres start|stop|restart|status|list" >&2
                exit 1
                ;;
        esac
        ;;
